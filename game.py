...
class CarRacing:
    ...
    def initialize(self):
        ...
        point = self.server.send('get')
        self.enemy_car_startx , self.enemy_car_starty = point[0],point[1]
    ...
    def run_car(self):
        ...
        while not self.crashed:
            ...
            x=self.server.send('get')
            print(x)
            self.enemy_car_startx,self.enemy_car_starty = x
            print(self.count)

            self.car(self.car_x_coordinate, self.car_y_coordinate)
            self.count += 1
            self.server.send((self.crashed,self.count),pick=True)
            if self.count%100==0:
                self.bg_speed +=1
